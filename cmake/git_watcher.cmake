...
function(GetGitState _working_dir)
    ...
    # Get output of git tags
    RunGitCommand(describe --tags ${object})
    if(NOT exit_code EQUAL 0)
        set(ENV{GIT_TAG} "unknown")
        set(ENV{GIT_VERSION} "?.?.?")
        set(ENV{GIT_VERSION_MAJOR} "?")
        set(ENV{GIT_VERSION_MINOR} "?")
        set(ENV{GIT_VERSION_PATCH} "?")
    else()
        set(ENV{GIT_TAG} "${output}")
        string(REGEX MATCH "^v([0-9]+)\\.([0-9]+)\\.([0-9]+)" GIT_VERSION "${output}")
        set(ENV{GIT_VERSION} "${GIT_VERSION}")
